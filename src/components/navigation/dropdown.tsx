...
import { Icon, IconProps, Popper } from '..';
  ...
  return (
    <DropdownContext.Provider value={{ isOpen }}>
      <Popper
        ...
        label={({ isOpen, ref, getItemProps, getReferenceProps }) =>
          React.cloneElement(label, {
            ref,
            ...getReferenceProps(getItemProps({ tabIndex: 0, role: 'button' })),
          })
        }
        content={({
          context,
                        ...
                        {group?.map((item, itemIndex) => {
                          ...
                          return (
                            <div
                              ...
                              ref={(element) =>
                                (listRef.current[currentIndex] = element)
                              }
                              role="option"
                              aria-selected={
...
interface DropdownContextAttributes {
  isOpen: boolean;
}

const DropdownContext = React.createContext<DropdownContextAttributes>(
  {} as DropdownContextAttributes
);

export const DropdownIcon: React.FC<Partial<IconProps>> = ({ name = 'IconChevronDown', ...props }) => {
